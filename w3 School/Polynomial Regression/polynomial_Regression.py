...
# Line for plotting the polynomial curve
myLine = np.linspace(min(x), max(x), 100)

# Plotting the data points and polynomial fit
plt.scatter(x, y, label="Data Points")
plt.plot(myLine, myModel(myLine), label="Polynomial Fit", color="blue")

# Indicating the predicted value with yellow lines
plt.vlines(17, ymin=min(y), ymax=speed, colors='yellow', label="Prediction Line (x=17)")
plt.hlines(speed, xmin=min(x), xmax=17, colors='yellow')

# Adding labels and legend
plt.xlabel("X values")
plt.ylabel("Y values")
plt.legend()
plt.title("Polynomial Fit and Prediction for x=17")
...
"""
    Now use Polynomial Regression in New Dataset
